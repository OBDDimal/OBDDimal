...
pub fn align_clauses(clauses:&Vec<Vec<i32>>, order:&Vec<u32>) -> Vec<usize> {
...
impl DDManager{
    ...
    pub fn from_instance(&mut self, instance:&mut Instance, order:Option<Vec<u32>>) -> u32 {


        if order.is_some() {
            self.order = order.unwrap();
            instance.clause_order = Some(align_clauses(&instance.clauses, &self.order));
        }

        let mut bdd = self.one();

        let mut index_giver = 
            (0..instance.clauses.len()).collect::<Vec<usize>>();

        if instance.clause_order.is_some() {
            index_giver = align_clauses(&instance.clauses, &self.order)
        }
        ...
        for i in iter {
            ...
            let mut cbdd = self.zero();
            for x in clause {

                let node = if *x < (0 as i32) {
                    self.nith_var(-x as u32)
                }else{
                    self.ith_var(*x as u32)
                };

                cbdd = self.or(node, cbdd);
            }

            bdd = self.and(cbdd, bdd);

            self.purge_retain(bdd);

            println!("{:?} ({:?}/{:?})", &self.nodes.len(), n, &instance.clauses.len());
            n += 1;
        }
    ...
    fn bootstrap(&mut self) -> () {
        ...
        let zero = Box::new(DDNode {id:0, var:0, low:0, high: 0, misc: 0});
        let one = Box::new(DDNode {id:1, var:0, low:1, high: 1, misc: 0});
    ...
    pub fn ith_var(&mut self, var:u32) -> u32 {
        let v = Box::new(DDNode {id: 0, var: var, low:0, high:1, misc:0});
    ...
    pub fn nith_var(&mut self, var:u32) -> u32 {
        let v = Box::new(DDNode {id: 0, var: var, low:1, high:0, misc:0});
    ...
    fn min_by_order(&self, fvar:u32, gvar:u32, hvar:u32) -> u32{
        ...
        let list:Vec<u32> = vec![fvar, gvar, hvar].into_iter().filter(|x| *x > 0).collect::<Vec<u32>>();

        // println!("{:?}", list);
        let tlist = list.iter().map(|&x| self.order[x as usize]).collect::<Vec<u32>>();
        // println!("{:?}", list);
    ...
    fn ite(&mut self, f:u32, g:u32, h:u32) -> u32 {
        ...
        match (f, g, h){
            ...
            (_, _, _) => {
                ...
                let node = Box::new(DDNode {id: 0, var: top, low: low, high: high, misc: 0});
    ...
    /// 
    /// 
    fn xor_prim(&mut self, vars:Vec<u32>) -> u32 {
       0 
    }
